...
class Manager
{
    ...
    {
        // Step 1: Fetch the image URL before deleting the DB record
        $stmt = $this->conn->prepare("SELECT link FROM pictures WHERE id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if (!$stmt->execute()) {
            error_log("Failed to delete picture: " . $stmt->error);
            $stmt->close();
            return ["success" => false, "message" => "Failed to delete the picture."];
        }
        ...
        $fileResult = !empty($imageUrl) ? $this->deleteImage($imageUrl) : null;

        return [
            "success"      => true,
            "data"         => $rowsAffected,
            "file_deleted" => $fileResult['success'] ?? false,
        ];
    }

    private function deleteImage(string $imageUrl): array
    {
        if (empty($imageUrl) || strpos($imageUrl, 'media/') !== 0) {
        ...
        if ($realFilePath === false || strpos($realFilePath, $mediaDir) !== 0) {
        ...
        try {
            if (isset($data['featured_image_url'])) {
                ...
                if ($stmt->affected_rows === 1) {
                    ...
                    // This might happen if the user doesn't exist or no change was made
                    return ['success' => false, 'message' => 'Image Failed to update the flyer'];
                }
            } else {
                ...
                if ($stmt->affected_rows === 1) {
                    ...
                    // This might happen if the user doesn't exist or no change was made
                    return ['success' => false, 'message' => ' status:' . $data['status'] . " ... " . ($stmt->affected_rows) . ': Failed to update the flyer'];
                }
            }
        } catch (Exception $e) {
            error_log("Update  error : " . $e->getMessage());
